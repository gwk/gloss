...
def main() -> None:
  ...
  if args.types:
    ...
    def cast_row(row:list[Any]) -> Sequence[Any]: return tuple(t(el) for t, el, in zip(types, row))
  else:
    def cast_row(row:list[Any]) -> Sequence[Any]: return row

  for i, raw_row in enumerate(reader(open(args.path, encoding=args.encoding))):
    try:
